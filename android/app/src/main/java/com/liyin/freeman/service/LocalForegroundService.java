...
public class LocalForegroundService extends Service {
    ...
    private static final String CHANNEL_NAME  = "KeepAlive Service";
    ...
    private void startService(){
        Log.d("[KeepAlive]","LocalForegroundService startService");


        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O){
            Log.d("[KeepAlive]","创建通知通道");
            // 创建通知通道
            ...
            Notification notification = builder.setOngoing(true)
                    .setContentTitle("FreeMan 正在运行")
                    .setContentText("保持后台运行，确保消息实时送达")
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        Log.d("[KeepAlive]","LocalForegroundService onStartCommand");
        // 绑定另外一个服务
        bindService();
    ...
    class Connection implements ServiceConnection {
        ...
        public void onServiceConnected(ComponentName name, IBinder service) {
            // 服务绑定成功时回调
            Log.d("[KeepAlive]","LocalForegroundService onServiceConnected");
        }
