...
import 'ui/my_app_routes.dart' show MyRouteGroup, kAboutRoute, kAllRoutes;
...
class MyAppSettings extends ChangeNotifier {
  ...
  //是否已注册
  static const _kAccountExistKey = 'ACCOUNT_EXIST';
  bool get isAccountExist=> _pref.getBool(_kAccountExistKey) ?? false;
  void setAccountExist(bool val) {
    _pref.setBool(_kAccountExistKey, val);
    notifyListeners();
  }

  // 登陆模式
  static const _kLoginModeKey = 'LOGIN_MODE';
  int get loginMode => _pref.getInt(_kLoginModeKey) ?? 0;
  set loginMode(int c) {
    _pref.setInt(_kLoginModeKey, c);
    notifyListeners();
  }

  //DHT网络状态
  static const _kDhtConnectStatusKey = 'DHT_CONNECT_STATUS';
  bool get isDhtConnected => _pref.getBool(_kDhtConnectStatusKey) ?? false;

  // ignore:avoid_positional_boolean_parameters
  void setDhtConnect(bool val) {
    _pref.setBool(_kDhtConnectStatusKey, val);
    notifyListeners();
  }
  ...
  int get currUserSex
  {
    return Global.user.sex;
  }

  // 新消息提示音
  static const _kMsgAudioKey = 'MSG_AUDIO';
  int get msgAudio => _pref.getInt(_kMsgAudioKey) ?? 0;
  set msgAudio(int c) {
    _pref.setInt(_kMsgAudioKey, c);
    notifyListeners();
  }
  ...
  bool get needCheckWhenApplyFriend
  {
    var mode =  Global.dhtClient.getAddMeMode();
    return Global.dhtClient.getAddMeMode() == 0 ? true : false;
  }

  void setCheckWhenApplyFriend(bool val)
  {
    Global.talker.debug(" setCheckWhenApplyFriend=$val");
    Global.dhtClient.setAddMeMode(val ? 0 : 1);
    notifyListeners();
  }
